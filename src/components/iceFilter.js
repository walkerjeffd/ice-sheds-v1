module.exports = function (evt) {
  return {
    props: ['id', 'range', 'variable', 'selected', 'getDim', 'getSelectedDim'],
    ...
    methods: {
      drawBrush: function () {
        // console.log('filter(' + this.id + '):drawBrush()', this.brush.empty(), this.range);
        if (this.brush.empty()) {
      ...
      render: function () {
        ...
        if (this.range && this.brush.empty()) {
          this.brush.extent(this.range);
          this.svg.select('g.brush').call(this.brush);
        }
